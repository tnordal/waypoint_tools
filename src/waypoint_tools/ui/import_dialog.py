...
class ImportFromControllerDialog(QDialog):
    ...
    def _load_missions(self) -> None:
        ...
        try:
            ...
            for mission_uuid in missions:
                # Check if already in database (by UUID or controller_uuid)
                existing = self.db.get_mission(mission_uuid)
                if not existing:
                    existing = self.db.get_mission_by_controller_uuid(mission_uuid)
    ...
    def _do_import(self) -> None:
        ...
        try:
            ...
            if imported_count > 0:
                new_count, updated_count = import_missions_from_folder(
                    missions_folder, from_controller=True
                )
